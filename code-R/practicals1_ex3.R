...
log_posterior_deriv <- function(y, x, beta, sigma, prior_sd) {
  ...
  deriv <- (t(X) %*% y - crossprod(X) %*% beta) / (sigma^2) + (beta[2] / (prior_sd^2))
...
hmc <- function(
  ...
  # assumes that the mass matrix will always be diagonal
  momentum_current <- rnorm(n = nparam, sd = diag(mass_matrix)) 
  
  # perform MCMC iterations
  for (m in seq(nsamples)) {
    # Leapfrog steps
    momentum_leapfrog <- rnorm(n = nparam, sd = diag(mass_matrix)) 
